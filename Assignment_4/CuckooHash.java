...
public class CuckooHash<K, V> {
	 ...
	 */
 	...
 	public void put(K key, V value) {
        ...
        while (count < CAPACITY) {
            ...
            curVal = displaced.getValue();

            useFirst = !useFirst;
            pos = useFirst ? hash1(curKey) : hash2(curKey);

            count++;
        }

        //If looped CAPACITY times →→ rehash
        rehash();
	...
	private void rehash() {
        ...
        for (int i = 0; i < oldCap; ++i) {
            if (old[i] != null) {
                put(old[i].getBucKey(), old[i].getValue());
            }
        }
    }
